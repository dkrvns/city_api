...
from city_api.application.interface.user.user import UserDeleter, UserReader, UserSaver
from city_api.domain.entities.user import UserDM
from city_api.infrastructure.db.models import User
...
class UserGateway(UserSaver, UserReader, UserDeleter):
    ...
    async def update(self, user: UserDM) -> None:
        stmt = update(User).where(and_(User.id == user.id)).values(**user.__dict__)
        await self._session.execute(stmt)
